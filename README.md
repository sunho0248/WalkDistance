...
WalkDistance의 소스 코드, 실행 파일, 문서, UI 및 자산은 **독점 소프트웨어**이며, 저작권은 2026 백선호에게 있습니다. 적용 법령에서 허용하는 경우를 제외하고, 별도의 서면 허가 없이 복제·수정·재배포·판매·역설계는 허용되지 않습니다. 설치 또는 사용하기 전에 [EULA](EULA.md)와 [LICENSE.txt](LICENSE.txt)를 확인하세요.

배포 ZIP에는 [EULA](EULA.md), [LICENSE.txt](LICENSE.txt), [THIRD-PARTY-NOTICES.txt](THIRD-PARTY-NOTICES.txt)가 함께 포함됩니다. NetTopologySuite 등 제3자 구성요소에는 각 구성요소의 라이선스가 적용되며, 관련 고지는 `THIRD-PARTY-NOTICES.txt`에서 확인할 수 있습니다.
...
시작할 때 한 세션에 한 번만 공개 GitHub Release를 확인합니다. 새 버전이 있으면 설치 여부를 묻고, **예**를 선택한 경우에만 `WalkDistance-win-x64.zip`를 내려받습니다. 최신 릴리즈 확인 단계의 네트워크 오류는 작업을 방해하지 않도록 알림 없이 건너뜁니다. 업데이트 설치를 선택한 뒤 발생한 다운로드·시간 초과·SHA-256 무결성 검증 오류는 안내합니다.
...
1. **파일 > DXF 열기**에서 도면을 열거나, DXF 파일 하나를 앱 창 어디에나 끌어다 놓습니다. DXF에 단위 정보가 없으면 원본 단위(mm, cm, m, inch, ft)를 선택합니다. `WD_Exit` 레이어의 지원 geometry가 있으면 출구로 자동 불러옵니다.
