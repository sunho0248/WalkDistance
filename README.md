...
2. `출구 지정 모드`를 켠 뒤 시작점과 끝점을 차례로 클릭해 출구 선분을 지정한다.
   벽이나 이미 그려진 출구 위를 클릭해도 항상 그대로 시작점/끝점으로 반영된다.
   첫 점을 찍은 뒤에는 마우스를 따라 미리보기 선이 표시되고, 끝점을 클릭하면
   즉시 선분이 확정된다. 이때 우클릭하면 지정 중인 선만 취소한다. 지정 중인 선이
   없을 때 우클릭은 완료된 출구를 절대 삭제하지 않으며, 선택된 출구가 있으면
   선택만 해제하고 없으면 아무 동작도 하지 않는다(항상 예측 가능한 동작).
   출구 지정 모드를 끄면 지정 중인 선이 취소된다.
...
1. 샘플 DXF를 열고 출구 지정 모드에서 두 번 클릭해 출구 선분을 만든다. 첫 클릭
   뒤 마우스를 움직일 때 미리보기 선이 따라오는지, 이미 그려진 벽/출구 위를 클릭해도
   즉시 시작점·끝점으로 반영되는지 확인한다.
...
3. 출구 지정 모드를 끄고 완료된 출구 선분 근처를 클릭해 파란색으로 강조 선택되는지
   ...
   셀 크기 입력 칸(`0.1`)을 클릭해 포커스를 옮기고 `Delete`를 눌러도 출구가 삭제되지
   않고 텍스트 편집에만 적용되는지 확인한다.
4. 기본 셀 크기 `0.1`로 계산해 히트맵, 빨간 최장거리 지점과 출구까지의 점선 경로가
   나타나는지 확인한다.
