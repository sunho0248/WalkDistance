...
- 거리맵·5m 색상 구간·등거리선과 주황색 `전체 최대` 경로는 인체 치수와 무관한 원본 도면의 전체 이동 가능 영역을 표시
- 자주색 `인체 적용 최대` 경로와 파란색 클릭 경로는 어깨너비를 적용하며, 좁은 통로의 통과 가능 여부를 반영
...
`디스턴스 맵`, `전체 최대/클릭 경로`, `인체 적용 최대 경로` 표시 옵션은 독립적입니다. 주황색 `전체 최대`은 도면 전체 거리맵의 실제 최원점까지의 경로이고, 자주색 `인체 적용 최대`은 현재 어깨너비로 사람이 통과할 수 있는 영역 안에서의 최장 경로입니다. 파란색은 클릭 지점에서 가장 가까운 출구까지의 인체 적용 경로입니다.
